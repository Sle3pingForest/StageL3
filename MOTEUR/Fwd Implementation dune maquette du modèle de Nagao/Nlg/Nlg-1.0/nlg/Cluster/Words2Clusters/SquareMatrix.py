...
class SquareMatrix(list):
	...
	def __init__(self, matrix, labels=None, visualization=False):
		...
		assert all( dimension == len(matrix[i]) for i in xrange(dimension) ), 'Not a square matrix.'
		list.__init__(self, matrix)
		self.dimension = dimension
		self.order = range(self.dimension)
		self.labels = labels
		if labels != None:
			self.labels = [ label.decode('utf8') for label in self.labels ]
		if __trace__: print >> sys.stderr, 'Labels = %s' % self.labels
	...
	def random_matrix(cls, dimension):
		...
		"""
		A = [ [ random.randint(0,1) for _ in xrange(options.dimension) ] for _ in xrange(options.dimension) ]
		# Symmetrize the matrix and fill the main diagonal with 0s.
		for i in xrange(options.dimension):
			A[i][i] = 0
			for j in xrange(i+1,options.dimension):
				A[j][i] = A[i][j]
		return SquareMatrix(A)

	def neighbors(self, v, indices=None):
		if indices == None: indices = xrange(self.dimension)
	...
	def _Bron_Kerbosch1(self, R, P, X, minsize=2, indices=None):
		...
		"""
		if __trace__: print >> sys.stderr, 'R = %s, P = %s, X = %s' % (R, P, X)
	...
	def covering_cliques(self, minsize=2):
		...
		"""
		if __trace__: print >> sys.stderr, '# Covering cliques...'
		...
		connections = { i: sum(self[i]) for i in xrange(len(self)) }
	...
	def visualize(self, title=None, indices=None):
		...
		if self.labels != None:
			if indices == None: indices = xrange(self.dimension)
			plt.yticks(range(self.dimension), [ self.labels[i] + (' %2d' % i) for i in xrange(self.dimension) ])
...
#		plt.set_ylabel(self.order)
	...
	def __repr__(self):
		return '\n'.join( '%s' % self[i] for i in xrange(self.dimension))
...
if __name__ == '__main__':
	...
	if __verbose__: print >> sys.stderr, '# Creating data...'
	A = SquareMatrix.random_matrix(options.dimension)
	print A
	t1 = time.time()
	if __verbose__: print >> sys.stderr, '# Clustering data...'
	for indices in A.covering_cliques(minsize=options.minsize):
		print list(indices)
	if __verbose__: print >> sys.stderr, '# Total time: %.2fs' % (time.time() - t1)
