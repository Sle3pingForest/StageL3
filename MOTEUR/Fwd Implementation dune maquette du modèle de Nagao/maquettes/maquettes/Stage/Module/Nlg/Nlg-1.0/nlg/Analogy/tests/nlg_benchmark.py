...
def random_word(alphabetsize, length):
	return ''.join([ chr(ord('a')+random.randint(0,alphabetsize-1)) for _ in xrange(length) ])
...
if __name__ == '__main__':
	...
	if not options.statistics:
		...
		print >> sys.stderr, '# Number of trials: %d' % options.repeat
		print >> sys.stderr, '# Success rate: %d%%' % success_result
		print >> sys.stderr, u'# Processing time: %dms' % time_result
	else:
		success_table, time_table = dict(), dict()
		for maxlen in xrange(1,options.strlen+1):
			success_table[maxlen], time_table[maxlen] = dict(), dict()
			for vocsize in xrange(1,options.vocsize+1):
		...
		msg += '\t'  +  '\t'.join([ '%d' % vocsize for vocsize in xrange(1,options.vocsize+1) ])  +  '\n'
		s += msg
		t += msg
		for maxlen in xrange(1,options.strlen+1):
			msg = '%d\t' % maxlen
			s += msg
			t += msg
			s += '\t'.join([ '%d' % success_table[maxlen][vocsize] for vocsize in xrange(1,options.vocsize+1) ])
			t += '\t'.join([ '%d' % time_table[maxlen][vocsize] for vocsize in xrange(1,options.vocsize+1) ])
			s += '\n'
			t += '\n'
		print s
		print t
